...
export class ClientComponent implements OnInit, OnChanges {
    ...
    public getDataTable(pageIndex: number, pageSize: number, sort: string, order: string, filter: string): void {
        ...
        this.block = true;
        Promise.all([Client.getAllPaged(offset, limit, sort, order, filter), Client.getCount(this.filter)])
            .then(
                values => {
                    this.block = false;
    ...
    public initSetting(): void {
        ...
        this.setting.cols = [
            {columnDef: 'photo',class: 'a10', header: 'client.placeholder.photo', type: 'img', cell: (row: any) => `${row.photo}`},
            {columnDef: 'name',class: 'a30', header: 'client.placeholder.name', type: 'text', cell: (row: any) => `${row.name}`},
            {columnDef: 'address',class: 'a30', header: 'client.placeholder.address', type: 'text', cell: (row: any) => `${row.address || ''}`},
            {columnDef: 'phone',class: 'a10', header: 'client.placeholder.phone', type: 'text', cell: (row: any) => `${row.phone}`},
     ...
     * onEditRow
     */
    public onEditRow(event: Client): void {
        this.client = event;
        this.router.navigate(['client-management/' + event.id]);
