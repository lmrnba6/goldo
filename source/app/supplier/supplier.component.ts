...
export class SupplierComponent implements OnInit, OnChanges {
    ...
    public getDataTable(pageIndex: number, pageSize: number, sort: string, order: string, filter: string): void {
        ...
        this.block = true;
        Promise.all([Supplier.getAllPaged(offset, limit, sort, order, filter), Supplier.getCount(this.filter)])
            .then(
                values => {
                    this.block = false;
     ...
     * onEditRow
     */
    public onEditRow(event: Supplier): void {
        this.supplier = event;
        this.router.navigate(['supplier-management/' + event.id]);
    }
